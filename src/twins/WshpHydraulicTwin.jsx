...
import { AlertTriangle, RotateCcw, Activity, Search, Gauge } from "lucide-react";
import { C, MONO, SANS, clamp, f1, f2, f0 } from "../theme.js";
import { Slider, Panel, Stat } from "../components.jsx";
...
function darcy(gpm, d, rough = 5e-6) {
...
const BASE = { demand: 1, addUnits: 0, vfdBldg: 100, vfdGeo: 100, Cf: 150, lenMul: 1, dia: {} };
...
function compute(S) {
  ...
  const segs = [];
  const mkBranch = (u, fl) => {
    const gpm = u.gpm * S.demand, d = dia[`WSHP-${u.id}`] ?? u.br, hf = hwPer100(gpm, d, S.Cf) * (25 * S.lenMul) / 100;
  ...
  TRUNKS.forEach((t) => {
    ...
    const d = dia[t.id] ?? t.d;
    const len = t.len * S.lenMul;
    segs.push({ id: t.id, grp: t.grp, d, gpm, len, v: vel(gpm, d), hf: hwPer100(gpm, d, S.Cf) * len / 100 });
  });
...
/* =================================== COMPONENT =================================== */
export default function WshpHydraulicTwin() {
  ...
  // layout helpers
  const ux2 = (i) => 300 + i * 138, uxy2 = 300;
  const uy1 = 690;
  const WSHPbox = (u, x, y, fl) => {
    ...
    return (
      <g key={u.id} transform={`translate(${x} ${y})`} style={{ cursor: "pointer" }} onClick={() => setSel(seg.id)}>
        <title>{`WSHP-${u.id} · ${f1(seg.v)} ft/s · click to inspect`}</title>
        ...
        <text x={0} y={11} textAnchor="middle" fontSize="11" fontFamily={MONO} fill={C.lab}>{u.ty} · {u.gpm} GPM · {seg.d}"</text>
    ...
    <div style={{ padding: "14px 16px 36px" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Stat label="Bldg Main" value={f1(R.bldgGPM)} unit="GPM" color={vColor(vel(R.bldgGPM, 3))} />
          <Stat label="Geo Main" value={f0(R.geoGPM)} unit="GPM" color={C.geo} />
          <Stat label="Max Velocity" value={f1(R.maxV.v)} unit="ft/s" color={vColor(R.maxV.v)} />
          <Stat label="Flags" value={R.flags.length} unit="" color={R.flags.length ? C.amber : C.green} />
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {last && <span style={{ fontFamily: MONO, fontSize: 11.5, color: C.amber }}>last · {last}</span>}
          <button onClick={reset} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", background: C.box, border: `1px solid ${C.line}`, color: C.txt2, borderRadius: 7, padding: "7px 12px", fontSize: 12.5 }}><RotateCcw size={13} /> Reset</button>
        </div>
      </div>
      ...
      <div style={{ background: C.canvas, border: `1px solid ${C.line}`, borderRadius: 12, padding: 8, marginTop: 12 }}>
        <svg viewBox="0 0 1480 820" style={{ width: "100%", height: "auto", display: "block" }}>
          ...
          <g onClick={() => setSel("WF")} style={{ cursor: "pointer" }}>
            ...
            <text x={110} y={572} textAnchor="middle" fontSize="11" fontFamily={MONO} fill={C.lab}>{f0(R.geoGPM)} GPM</text>
          </g>
          ...
          <text x={640} y={552} fontSize="11.5" fontFamily={MONO} fill={C.lab}>Geo main 6"–8" · {f0(R.geoGPM)} GPM · {f1(vel(R.geoGPM, 8))} ft/s (8")</text>
          ...
          <text x={740} y={806} fontSize="11.5" fontFamily={MONO} fill={C.dim}>branch/header width ∝ flow · color = velocity (blue &lt;2 · green ≤4 · yellow ≤8 · amber ≤10 · red &gt;10 ft/s) · click any element to inspect/resize</text>
        </svg>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px,1fr))", gap: 12, marginTop: 12 }}>
        {/* selected element */}
        <Panel icon={Gauge} title={selSeg ? `Inspect · ${selSeg.id}` : sel && /P-/.test(sel) ? `Pump · ${sel}` : "Select an element"}>
          {selSeg ? (<div>
            <Slider label="Pipe diameter" value={selSeg.d} min={0.75} max={12} step={0.25} unit="in" accent={C.blue}
              onChange={(v) => setDiameter(selSeg.id, v)} note="ΔP ∝ 1/d⁴·⁸⁷ · velocity ∝ 1/d²" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontFamily: MONO, fontSize: 12.5, marginTop: 4 }}>
              ...
              <span style={{ color: C.txt2 }}>Re <b style={{ color: C.ink }}>{f0(darcy(selSeg.gpm, selSeg.d).Re)}</b></span>
            </div>
        ...
        {/* what-if */}
        <Panel icon={Activity} title="What-if">
          ...
          <div style={{ fontFamily: MONO, fontSize: 12, color: C.txt2, marginTop: 4 }}>bldg main <b style={{ color: vColor(vel(R.bldgGPM, 3)) }}>{f1(R.bldgGPM)}</b> GPM @ {f1(vel(R.bldgGPM, 3))} ft/s (3") · geo <b style={{ color: C.ink }}>{f0(R.geoGPM)}</b> GPM</div>
        </Panel>

        {/* validation */}
        <Panel icon={AlertTriangle} title={`Validation (${R.flags.length})`}>
        ...
        {/* query */}
        <Panel icon={Search} title="Engineering query (computed from the model)" span>
